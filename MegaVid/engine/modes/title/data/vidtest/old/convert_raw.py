...
y_size = int(raw_input("FULL Vertical size? ")) #(y_l+y_r)
...
while frames_vid:
  ...
  if chksumfrm == lastchks:
    ...
    if frames_vid == 1:
      ...
      out_art.seek(lastartaddr)
      d1 = -1
      out_art.write( chr(int((d1&0xFF000000)>>0x18)&0xFF) + chr(int((d1&0xFF0000)>>0x10)&0xFF) + chr(int((d1&0xFF00)>>8)&0xFF) + chr(int(d1)&0xFF) )
      out_art.seek(pushpop_artaddr)
      
    pushpop_artaddr = out_art.tell()
    out_art.seek(samefrmcntr)
    d1 = same_frame
    out_art.write( chr(int((d1&0xFF000000)>>0x18)&0xFF) + chr(int((d1&0xFF0000)>>0x10)&0xFF) + chr(int((d1&0xFF00)>>8)&0xFF) + chr(int(d1)&0xFF) )
    out_art.seek(pushpop_artaddr)
    ...
    out_map.seek(addr_mapsize)
    a = pushpop_mapaddr
    if frames_vid == 1:
      a = -1
    d1 = a
    out_map.write( chr(int((d1&0xFF000000)>>0x18)&0xFF) + chr(int((d1&0xFF0000)>>0x10)&0xFF) + chr(int((d1&0xFF00)>>8)&0xFF) + chr(int(d1)&0xFF) )
    out_map.seek(pushpop_mapaddr)
    addr_mapsize = out_map.tell()
  
    pushpop_artaddr = out_art.tell()
    out_art.seek(addr_artsize)
    a = pushpop_artaddr
    if frames_vid == 1:
      a = -1
    d1 = a
