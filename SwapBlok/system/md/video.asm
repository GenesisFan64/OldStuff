...
.yloop:
		move.l	#0,(a6)				; Y scroll value(s)
		dbf	d5,.yloop

		moveq	#0,d0
		move.w	#$7FF,d1
		move.b	(RAM_VdpCache+5).l,d2		; Sprites
		andi.w	#%1111111,d2
		lsl.w	#8,d2
		lsl.w	#1,d2
		bsr	Video_Fill
		...
		move.w	#$FFF,d1
.smlwdw:
		move.b	(RAM_VdpCache+3).l,d2		; Window
		andi.w	#%111110,d2
		lsl.w	#8,d2
		lsl.w	#2,d2
		bra	Video_Fill
...
; Video_LoadPal_Fade
; 
; Load palette to cache fadein/fadeout
; 
; Input:
; a0 - Palette data
; d0 - Start position
...
; --------------------------------------------------------
; Video_FadePal_In
; Fade in palette from target palette
; 
; NOTE: Color dots will be shown on screen
; 
; Input:
...
; 
; NOTES:
; Use DMA externally to transfer the output
; result:
; 	lea	(vdp_ctrl),a6
; 	move.l	#$94009340,(a6)
...
.wait:		move.w	(a6),d5
		...
		beq.s	.wait
		tst.w	d4
		bne.s	Video_PalFade_In
		rts
.do_it:
		move.w	#-1,d4
		sub.w	#1,(RAM_FadeTimer).w
		bpl	.timer
		move.w	d2,(RAM_FadeTimer).w
		lea	(RAM_FadeBuff),a4
		lea	(RAM_FadeTarget),a5
		moveq	#0,d5
		move.w	d0,d5
		move.w	d1,d7
		move.w	d7,d4
...
.wait:		move.w	(a6),d5
		btst	#bitVBlnk,d5
		beq.s	.wait
		cmp.w	(RAM_FadeTimer).w,d2
		bne.s	.lowr
		move.w	#$8100,d5
		move.b	(RAM_VdpCache+1).w,d5
		bset	#4,d5
		move.w	d5,(a6)
		move.l	#$94009340,(a6)
		move.l	#$96009500|(RAM_FadeTarget<<7&$FF0000)+(RAM_FadeTarget>>1&$FF),(a6)
...
; --------------------------------------------------------
; Shared: pick layer / x pos / y pos from d0 and
; set next-line size
; 
; d0 - locate(layer,x,y)
; 
...
; 
; If porting to 32X: you need to move these
; routines to RAM
; --------------------------------------------------------
